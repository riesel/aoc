export default function (rawData: string) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (
      (getBeginning(data[i][0]) <= getBeginning(data[i][1]) && getBeginning(data[i][1]) <= getEnding(data[i][0])) ||
      (getBeginning(data[i][0]) <= getEnding(data[i][1]) && getEnding(data[i][1]) <= getEnding(data[i][0])) ||
